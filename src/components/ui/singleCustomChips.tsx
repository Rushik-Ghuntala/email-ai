...
type CustomChipsProps = {
  ...
  label?: string
  description?: string
  onSelect: (name: string, item: string | string[]) => void // Allow both string and string[] here
  onSelect2?: (step: number, name: string, item: string) => void
  name: string
  selected?: string | string[] // Allow both string and string[]
  isInvalid?: boolean
  currentStep?: number
  setedStep?: number
}

export const SingleCustomChips = ({
  id,
  data,
  label,
  description,
  ...
  const [selectedItems, setSelectedItems] = useState<string[]>(
    Array.isArray(selected) ? selected : selected ? [selected] : [] // Handle both string and string[] cases
  )
  ...
  const handleItemClick = (item: string) => {
    if (Array.isArray(selected)) {
      // Handle array of selected items (for tone)
      ...
      onSelect(name, newSelectedItems) // Pass the array of selected items
    } else {
      // Handle single selected item (for size)
      setSelectedItems([item])
      onSelect(name, item) // Pass the single string for size selection
    }
  }

  useEffect(() => {
    if (currentStep && setedStep && currentStep <= setedStep) {
      setSelectedItems([]) // Clear selections when the step condition is met
    }
  }, [currentStep, setedStep])
